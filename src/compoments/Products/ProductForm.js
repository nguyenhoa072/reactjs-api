...
import { add_product_request, get_product_request, update_product_request } from './../../store/actions/products';
    ...
    return (
      <Fragment>
        <div className="card border-primary">
          <h5 className="card-header text-white bg-primary">{this.state.id ? 'Sửa sản phẩm' : 'Thêm sản phẩm'}</h5>
          <div className="card-body">
            <form onSubmit={this.on_save}>
              <div className="form-row">
                <div className="form-group col-md-6">
                  <label htmlFor="inputEmail4">Tên</label>
                  <input onChange={this.input_change} type="text" value={name} className="form-control" name="name" placeholder="" required />
                </div>
                <div className="form-group col-md-6">
                  <label htmlFor="inputPassword4">Giá</label>
                  <input onChange={this.input_change} type="text" value={price} className="form-control" name="price" placeholder="" />
                </div>
              </div>
              <div className="form-group">
                <label htmlFor="inputAddress">Mô tả</label>
                <FroalaEditor config={this.config} tag='textarea' onModelChange={this.handleModelChange} model={content} name={content} />
              </div>
              <div className="form-group">
                <div className="custom-control custom-checkbox">
                  <input value={status} onChange={this.input_change} type="checkbox" className="custom-control-input" name="status" id="status" checked={status} />
                  <label className="custom-control-label" htmlFor="status">Kích hoạt</label>
                </div>
              ...
              <div className="row">
                <div className="col-3 offset-6">
                  <button type="button" onClick={() => window.history.back()} className="btn btn-secondary btn-block">Hủy</button>
                </div>
                <div className="col-3">
